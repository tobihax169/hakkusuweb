  ...
  <div>
    ...
    <div class="grid grid-cols-1 sm:grid-cols-4 gap-6 mb-8">
      ...
      <div class="card p-6">
        <div class="flex items-center justify-between">
          <div>
            <p class="text-sm font-medium text-gray-500 dark:text-gray-400">Đang hiển thị</p>
      ...
      <div class="card p-6">
        <div class="flex items-center justify-between">
          <div>
            <p class="text-sm font-medium text-gray-500 dark:text-gray-400">Lượt xem</p>
            <p class="text-2xl font-bold text-gray-900 dark:text-white mt-1">{{ formatNumber(stats.views) }}</p>
          </div>
      ...
      <div class="card p-6">
        <div class="flex items-center justify-between">
          <div>
            ...
            <p class="text-2xl font-bold text-gray-900 dark:text-white mt-1">{{ stats.scheduled }}</p>
          </div>
    ...
    <div class="card">
      <div class="overflow-x-auto">
        <table class="w-full">
          <thead class="bg-gray-50 dark:bg-gray-800">
            <tr>
              <th class="px-6 py-4 text-left text-xs font-medium text-gray-500 uppercase">Thông báo</th>
              <th class="px-6 py-4 text-left text-xs font-medium text-gray-500 uppercase">Loại</th>
              <th class="px-6 py-4 text-left text-xs font-medium text-gray-500 uppercase">Đối tượng</th>
              <th class="px-6 py-4 text-left text-xs font-medium text-gray-500 uppercase">Thời gian</th>
              <th class="px-6 py-4 text-left text-xs font-medium text-gray-500 uppercase">Lượt xem</th>
              <th class="px-6 py-4 text-left text-xs font-medium text-gray-500 uppercase">Trạng thái</th>
              <th class="px-6 py-4 text-left text-xs font-medium text-gray-500 uppercase">Thao tác</th>
            </tr>
          </thead>
          <tbody class="divide-y divide-gray-200 dark:divide-gray-700">
            <tr v-for="item in filteredAnnouncements" :key="item.id" class="hover:bg-gray-50 dark:hover:bg-gray-800/50">
              <td class="px-6 py-4">
              ...
              <td class="px-6 py-4 text-sm text-gray-600 dark:text-gray-400">
                ...
                <div v-if="item.endDate" class="text-xs text-gray-500">đến {{ formatDate(item.endDate) }}</div>
              </td>
              <td class="px-6 py-4 text-sm text-gray-600 dark:text-gray-400">
      ...
      <div v-if="filteredAnnouncements.length === 0" class="text-center py-12">
        <MegaphoneIcon class="w-16 h-16 mx-auto text-gray-300 dark:text-gray-600 mb-4" />
        <p class="text-gray-500 dark:text-gray-400">Chưa có thông báo nào</p>
      </div>
    </div>
    ...
    <div v-if="showCreateModal || editingItem" class="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/50">
      <div class="bg-white dark:bg-gray-800 rounded-2xl w-full max-w-2xl max-h-[90vh] overflow-y-auto">
        <div class="p-6 border-b border-gray-200 dark:border-gray-700 flex items-center justify-between">
          ...
          <button @click="closeModal" class="p-2 rounded-lg hover:bg-gray-100 dark:hover:bg-gray-700">
        ...
        <form @submit.prevent="saveAnnouncement" class="p-6 space-y-6">
          <div>
            ...
            <input
              ...
              type="text"
              required
              class="w-full px-4 py-2 rounded-lg border border-gray-200 dark:border-gray-600 bg-white dark:bg-gray-800 text-gray-900 dark:text-white"
          ...
          <div>
            ...
            <textarea
              ...
              rows="5"
              required
              class="w-full px-4 py-2 rounded-lg border border-gray-200 dark:border-gray-600 bg-white dark:bg-gray-800 text-gray-900 dark:text-white resize-none"
          ...
          <div class="grid grid-cols-2 gap-4">
            ...
            <div>
              <label class="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-2">Đối tượng nhận</label>
              <select
                v-model="form.target"
                class="w-full px-4 py-2 rounded-lg border border-gray-200 dark:border-gray-600 bg-white dark:bg-gray-800 text-gray-900 dark:text-white"
              >
          ...
          <div class="grid grid-cols-2 gap-4">
            ...
            <div>
              <label class="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-2">Ngày kết thúc (tùy chọn)</label>
              <input
          ...
          <div class="flex items-center gap-2">
            ...
            />
            <label for="pinned" class="text-sm text-gray-700 dark:text-gray-300">Ghim lên đầu trang</label>
          </div>
...
const getTypeClass = (type) => {
  const classes = {
    ...
    general: 'px-3 py-1 rounded-full text-xs font-medium bg-gray-100 dark:bg-gray-700 text-gray-600 dark:text-gray-400'
  };
...
const getStatusClass = (status) => {
  const classes = {
    ...
    draft: 'px-3 py-1 rounded-full text-xs font-medium bg-gray-100 dark:bg-gray-700 text-gray-600 dark:text-gray-400',
