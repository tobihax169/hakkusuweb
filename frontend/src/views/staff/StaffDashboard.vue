<template>
  <div class="space-y-8">
    ...
    <div class="flex items-center justify-between">
      <div>
        <h1 class="text-3xl font-bold bg-gradient-to-r from-slate-900 via-primary-600 to-indigo-600 dark:from-white dark:via-primary-400 dark:to-purple-400 bg-clip-text text-transparent">
          {{ $t('staff.dashboard') }}
        </h1>
        <p class="text-slate-500 dark:text-slate-400 mt-1">Quản lý hỗ trợ và phê duyệt</p>
      </div>
    ...
    <div class="bg-white/80 dark:bg-gray-800/80 backdrop-blur-xl rounded-2xl shadow-xl border border-slate-100 dark:border-gray-700 overflow-hidden">
      ...
      <div class="p-6 grid grid-cols-1 sm:grid-cols-3 gap-4">
        <router-link to="/staff/tickets" class="group flex items-center gap-4 p-5 rounded-xl bg-gradient-to-br from-rose-50 to-red-50 dark:from-rose-900/20 dark:to-red-900/20 border border-rose-100 dark:border-rose-800 hover:shadow-lg hover:shadow-rose-500/20 transition-all duration-300">
          ...
          <div>
            <p class="font-bold text-slate-900 dark:text-white group-hover:text-rose-600 dark:group-hover:text-rose-400 transition-colors">Xử lý Tickets</p>
            <p class="text-sm text-slate-500">{{ stats.pendingTickets }} đang chờ xử lý</p>
          </div>
          <ArrowRightIcon class="w-5 h-5 text-slate-400 group-hover:text-rose-500 group-hover:translate-x-1 transition-all ml-auto" />
        </router-link>
        ...
        <router-link to="/staff/approvals" class="group flex items-center gap-4 p-5 rounded-xl bg-gradient-to-br from-blue-50 to-cyan-50 dark:from-blue-900/20 dark:to-cyan-900/20 border border-blue-100 dark:border-blue-800 hover:shadow-lg hover:shadow-blue-500/20 transition-all duration-300">
          ...
          <div>
            <p class="font-bold text-slate-900 dark:text-white group-hover:text-blue-600 dark:group-hover:text-blue-400 transition-colors">Duyệt đơn</p>
            <p class="text-sm text-slate-500">{{ stats.pendingSellers + stats.pendingProducts }} chờ duyệt</p>
          </div>
          <ArrowRightIcon class="w-5 h-5 text-slate-400 group-hover:text-blue-500 group-hover:translate-x-1 transition-all ml-auto" />
        </router-link>

        <router-link to="/admin/announcements" class="group flex items-center gap-4 p-5 rounded-xl bg-gradient-to-br from-purple-50 to-pink-50 dark:from-purple-900/20 dark:to-pink-900/20 border border-indigo-100 dark:border-purple-800 hover:shadow-lg hover:shadow-purple-500/20 transition-all duration-300">
          <div class="w-14 h-14 rounded-xl bg-gradient-to-br from-indigo-500 to-blue-500 flex items-center justify-center shadow-lg shadow-purple-500/30 group-hover:scale-110 transition-transform">
            <MegaphoneIcon class="w-7 h-7 text-white" />
          </div>
          <div>
            <p class="font-bold text-slate-900 dark:text-white group-hover:text-indigo-600 dark:group-hover:text-purple-400 transition-colors">Thông báo</p>
            <p class="text-sm text-slate-500">Gửi thông báo hệ thống</p>
          </div>
          <ArrowRightIcon class="w-5 h-5 text-slate-400 group-hover:text-purple-500 group-hover:translate-x-1 transition-all ml-auto" />
        </router-link>
      </div>
    </div>
    ...
    <div class="bg-white/80 dark:bg-gray-800/80 backdrop-blur-xl rounded-2xl shadow-xl border border-slate-100 dark:border-gray-700 overflow-hidden">
      <div class="p-6 border-b border-slate-200 dark:border-gray-700 flex items-center justify-between">
        ...
        <button class="text-sm text-blue-600 hover:text-primary-700 dark:text-primary-400">Xem tất cả</button>
      </div>
      <div class="p-6">
        ...
        <div v-else-if="recentActivity.length === 0" class="text-center py-12">
          <div class="w-20 h-20 mx-auto mb-4 rounded-full bg-slate-100 dark:bg-gray-700 flex items-center justify-center">
            <ClockIcon class="w-10 h-10 text-slate-400" />
          </div>
          <p class="text-slate-500 dark:text-slate-400">Chưa có hoạt động nào</p>
        </div>
        <div v-else class="space-y-4">
          <div v-for="(activity, index) in recentActivity" :key="activity.id" class="flex items-start gap-4 p-4 rounded-xl bg-gradient-to-r from-gray-50 to-transparent dark:from-gray-700/50 dark:to-transparent hover:from-gray-100 dark:hover:from-gray-700 transition-colors border-l-4" :class="getActivityBorderClass(activity.type)">
            ...
            <div class="flex-1 min-w-0">
              <p class="font-bold text-slate-900 dark:text-white">{{ activity.title }}</p>
              ...
              <div class="flex items-center gap-2 mt-2">
                ...
                <span class="text-xs text-slate-400">{{ formatDate(activity.createdAt) }}</span>
              </div>
            </div>
          </div>
        </div>
...
const getActivityIconClass = (type) => {
  ...
  return classes[type] || 'bg-slate-100 dark:bg-gray-800';
};
...
const getActivityBadgeClass = (type) => {
  ...
  return classes[type] || 'bg-slate-100 dark:bg-gray-700 text-slate-700';
};
