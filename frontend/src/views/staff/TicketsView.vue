  ...
  <div>
    <div class="flex items-center justify-between mb-6">
      ...
      <div class="flex items-center gap-4">
        <select
          ...
          class="px-4 py-2 rounded-lg border border-gray-200 dark:border-gray-700 bg-white dark:bg-gray-800 text-gray-900 dark:text-white"
        >
    ...
    <div class="card">
        ...
        <table class="w-full">
          <thead class="bg-gray-50 dark:bg-gray-800">
            <tr>
              ...
              <th class="px-6 py-4 text-left text-xs font-medium text-gray-500 dark:text-gray-400 uppercase tracking-wider">Tiêu đề</th>
              <th class="px-6 py-4 text-left text-xs font-medium text-gray-500 dark:text-gray-400 uppercase tracking-wider">Người gửi</th>
              <th class="px-6 py-4 text-left text-xs font-medium text-gray-500 dark:text-gray-400 uppercase tracking-wider">Mức độ</th>
              <th class="px-6 py-4 text-left text-xs font-medium text-gray-500 dark:text-gray-400 uppercase tracking-wider">Trạng thái</th>
              <th class="px-6 py-4 text-left text-xs font-medium text-gray-500 dark:text-gray-400 uppercase tracking-wider">Người xử lý</th>
              <th class="px-6 py-4 text-left text-xs font-medium text-gray-500 dark:text-gray-400 uppercase tracking-wider">Thời gian</th>
          ...
          <tbody class="divide-y divide-gray-200 dark:divide-gray-700">
            <tr v-for="ticket in filteredTickets" :key="ticket.id" class="hover:bg-gray-50 dark:hover:bg-gray-800/50">
              ...
              <td class="px-6 py-4 text-sm text-gray-900 dark:text-white">
                ...
                <div class="text-gray-500 text-xs">{{ ticket.category }}</div>
              </td>
      ...
      <div v-if="filteredTickets.length === 0" class="text-center py-12">
        <TicketIcon class="w-16 h-16 mx-auto text-gray-300 dark:text-gray-600 mb-4" />
        <p class="text-gray-500 dark:text-gray-400">Không có ticket nào</p>
      </div>
    </div>
    ...
    <div v-if="selectedTicket" class="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/50">
      <div class="bg-white dark:bg-gray-800 rounded-2xl w-full max-w-3xl max-h-[90vh] overflow-hidden">
        <div class="p-6 border-b border-gray-200 dark:border-gray-700 flex items-center justify-between">
          <div>
            <h2 class="text-xl font-bold text-gray-900 dark:text-white">#{{ selectedTicket.id }} - {{ selectedTicket.title }}</h2>
            <p class="text-sm text-gray-500 mt-1">{{ selectedTicket.category }}</p>
          </div>
          <button @click="selectedTicket = null" class="p-2 rounded-lg hover:bg-gray-100 dark:hover:bg-gray-700">
        ...
        <div class="p-6 overflow-y-auto max-h-[60vh]">
          <!-- Ticket Info -->
          <div class="grid grid-cols-2 gap-4 mb-6 p-4 bg-gray-50 dark:bg-gray-700 rounded-xl">
            <div>
              <p class="text-sm text-gray-500">Người gửi</p>
              <p class="font-medium text-gray-900 dark:text-white">{{ selectedTicket.user.name }}</p>
              <p class="text-sm text-gray-500">{{ selectedTicket.user.email }}</p>
            </div>
            <div>
              <p class="text-sm text-gray-500">Người xử lý</p>
              <p class="font-medium text-gray-900 dark:text-white">{{ selectedTicket.assignedTo || 'Chưa phân công' }}</p>
            </div>
            <div>
            ...
            <div>
              ...
              <select v-model="selectedTicket.status" @change="updateStatus(selectedTicket)" class="mt-1 px-3 py-1 rounded-lg border border-gray-200 dark:border-gray-600 bg-white dark:bg-gray-800 text-sm">
                <option value="open">Đang mở</option>
                <option value="pending">Đang chờ</option>
                <option value="resolved">Đã giải quyết</option>
                <option value="closed">Đã đóng</option>
          ...
          <div class="space-y-4">
            <div v-for="message in selectedTicket.messages" :key="message.id" :class="['flex gap-4', message.isStaff ? 'flex-row-reverse' : '']">
              <div class="w-10 h-10 rounded-full flex-shrink-0 flex items-center justify-center text-sm font-bold" :class="message.isStaff ? 'bg-blue-500 text-white' : 'bg-gray-200 dark:bg-gray-600 text-gray-700 dark:text-gray-300'">
                {{ message.author.charAt(0) }}
              </div>
              <div :class="['flex-1 p-4 rounded-2xl', message.isStaff ? 'bg-blue-100 dark:bg-blue-900/30 text-blue-900 dark:text-blue-100' : 'bg-gray-100 dark:bg-gray-700 text-gray-900 dark:text-white']">
        ...
        <div class="p-6 border-t border-gray-200 dark:border-gray-700">
          <div class="flex gap-4">
            <textarea
              ...
              class="flex-1 px-4 py-3 rounded-xl border border-gray-200 dark:border-gray-600 bg-white dark:bg-gray-800 text-gray-900 dark:text-white resize-none"
            />
            <button
              @click="sendReply"
...
const getStatusClass = (status) => {
  const classes = {
    ...
    closed: 'px-3 py-1 rounded-full text-xs font-medium bg-gray-100 dark:bg-gray-700 text-gray-600 dark:text-gray-400'
  };
